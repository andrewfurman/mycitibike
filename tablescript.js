...
window.onload=function(){
	...
	var listOfRides = "";
		...
		listOfRides += '<table id="myTable" class="tablesorter"><thead> <tr> <th>Start Date</th><th>Start Dock</th> <th>End Date</th> <th>End Dock</th>'; 
		listOfRides += '<th>Duration (minutes)</th> </tr> </thead> <tbody>';
		for(var i = 0; i<numberOfRides; i++){
			startDate = Date.parseString(rides[i].StartDateTime,'MM/dd/yyyy h:mm:ss a');
			endDate = Date.parseString(rides[i].EndTime,'MM/dd/yyyy h:mm:ss a');
			difference = endDate - startDate;
			diffMins = Math.round(((difference % 86400000) % 3600000) / 60000); 
			listOfRides += '<tr><td>'+rides[i].StartDateTime+'</td> <td>'+rides[i].StartStation+'</td>  <td>'+rides[i].EndTime+'</td>';  
			listOfRides += '<td>'+rides[i].EndLocation+'</td><td>'+diffMins+'</td> </tr>';
		}
		listOfRides += '</tbody> 	</table>';
